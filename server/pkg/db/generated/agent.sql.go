...
func (q *Queries) CancelAgentTasksByIssue(ctx context.Context, issueID pgtype.UUID) ([]TaskRun, error) {
...
// Chat tasks (task_id IS NULL) use chat_session_id for serialization instead.
...
SELECT id, agent_id, task_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, last_heartbeat_at, trigger_summary, force_fresh_session FROM task_run
WHERE id = $1
`
...
// (agent_id, task_id) pair, used for session resumption. We accept both
...
// the agent has finished and the issue exists. Guarded by `task_id IS NULL`
...
func (q *Queries) ListActiveTasksByIssue(ctx context.Context, issueID pgtype.UUID) ([]TaskRun, error) {
...
SELECT id, agent_id, task_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, autopilot_run_id, attempt, max_attempts, parent_task_id, failure_reason, last_heartbeat_at, trigger_summary FROM task_run
...
func (q *Queries) ListTasksByIssue(ctx context.Context, issueID pgtype.UUID) ([]TaskRun, error) {
