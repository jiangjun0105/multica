...
func TestCreateComment_AgentWithWrongParentRejected(t *testing.T) {
	...
	t.Cleanup(func() {
		testPool.Exec(ctx, `DELETE FROM task WHERE id = $1`, freshTaskID)
	...
	if err := testPool.QueryRow(ctx,
		`INSERT INTO task (workspace_id, number, title, creator_type, creator_id, agent_id, runtime_id, issue_id, status, queue_priority, trigger_comment_id)
		 VALUES ($1, 1001, 'test-task', 'agent', $2, $2, $3, $4, 'queued', 0, $5) RETURNING id`,
		testWorkspaceID, agentID, runtimeID, issueA, freshParent.ID,
