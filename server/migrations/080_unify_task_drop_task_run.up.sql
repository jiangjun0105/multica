...
-- 1. Drop current_run_id (was FK to task_run; no longer needed).
ALTER TABLE task DROP COLUMN current_run_id;

-- 2. Expand status CHECK from 5 to 10 values.
...
-- 3. Add execution columns (from task_run).
...
-- 4. Add crew columns (for auto_agent multi-agent execution).
...
-- 5. Add indexes for execution queries.
...
-- 6. Migrate task_message FK from task_run to task.
...
-- 7. Migrate task_usage FK from task_run to task.
